/**
 * HTML parser for Taiwanese legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Taiwanese legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Taiwanese statute.
 *
 ...
 *   </div>
 */
export function parseTaiwaneseHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Taiwanese acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Taiwanese legal text.
 *
 * Taiwanese definitions typically use patterns like:
