#!/usr/bin/env tsx
/**
 * Drift detection for Taiwanese Law MCP.
 *
 * Checks if upstream Taiwan legal source snippets have changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Taiwanese Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Taiwanese-Law-MCP/1.0 drift-detect' },
      });
