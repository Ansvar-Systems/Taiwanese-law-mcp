#!/usr/bin/env tsx
/**
 * Paid-tier database builder for Taiwan Law MCP server.
 *
 ...
 *   3. Updates db_metadata to reflect the premium tier
 *
 * Usage: npx tsx scripts/premium-ingestion/taiwanese/build-db-paid.ts
 *        npx tsx scripts/premium-ingestion/taiwanese/build-db-paid.ts --db /path/to/database.db
 */
...
CREATE TABLE IF NOT EXISTS preparatory_works (
  ...
  category TEXT,
  proposer TEXT,
  source TEXT NOT NULL DEFAULT 'lyapi',
  inserted_at TEXT DEFAULT (datetime('now'))
);
...
CREATE INDEX IF NOT EXISTS idx_prep_works_term ON preparatory_works(term_no);
...
function buildPaidTier(): void {
  ...
  console.log('Building paid-tier extensions for Taiwan Law MCP...\n');
  ...
  const updateMeta = db.transaction(() => {
    ...
    upsertMeta.run('jurisdiction', 'TW');
    upsertMeta.run('paid_tables', 'case_law,case_law_full,preparatory_works,preparatory_works_full,agency_guidance');
    upsertMeta.run('source_case_law', 'judicial_yuan');
    upsertMeta.run('source_prep_works', 'lyapi_v2');
    upsertMeta.run('source_agency', 'ncc.gov.tw,ftc.gov.tw');
  });
  updateMeta();
