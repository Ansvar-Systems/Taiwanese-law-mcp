#!/usr/bin/env tsx
/**
 * Taiwanese Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://law.moj.gov.tw';
const PORTAL_NAME = 'Laws and Regulations Database (Ministry of Justice)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/taiwanese-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Taiwanese Law MCP — Data Freshness Check');
