/**
 * search_eu_implementations — Search EU directives/regulations referenced by Taiwanese legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_taiwanese_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  taiwanese_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as taiwanese_statute_count
  ...
  if (input.has_taiwanese_implementation) {
    sql += ' HAVING taiwanese_statute_count > 0';
  }
