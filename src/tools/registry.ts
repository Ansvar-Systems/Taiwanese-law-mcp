/**
 * Tool registry for Taiwanese Law MCP Server.
...
import { getTaiwaneseImplementations, type GetTaiwaneseImplementationsInput } from './get-taiwanese-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Official Gazette (Republic of China). ' +
    ...
    'Call this FIRST when you need to understand what Taiwanese legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Taiwanese statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from a Taiwanese statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Taiwanese legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Taiwanese statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Taiwanese?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Taiwanese legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Taiwanese statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get international legal instruments that a Taiwanese statute references or aligns with. ' +
      'Taiwanese laws may reference international standards and conventions ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Taiwanese statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_taiwanese_implementations',
    description:
      'Find all Taiwanese statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Taiwanese statutes. ' +
      'Taiwanese may reference international legal instruments where relevant.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Taiwanese legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_taiwanese_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Taiwanese legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Taiwanese statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Taiwanese statute or provision. ' +
      ...
      'Taiwanese is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Taiwanese statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_taiwanese_implementations':
          result = await getTaiwaneseImplementations(db, args as unknown as GetTaiwaneseImplementationsInput);
          break;
