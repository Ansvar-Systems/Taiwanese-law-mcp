/**
 * get_taiwanese_implementations — Find Taiwanese statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetTaiwaneseImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface TaiwaneseImplementationResult {
  document_id: string;
...
export async function getTaiwaneseImplementations(
  db: InstanceType<typeof Database>,
  input: GetTaiwaneseImplementationsInput,
): Promise<ToolResponse<TaiwaneseImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as TaiwaneseImplementationResult[];
